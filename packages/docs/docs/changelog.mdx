...
- Removed `environment.onStartRenamingItem` and `environment.onAbortRenamingItem`, since they were not working anyways
  and the renaming state is owned by a tree instance, not its environment. Use the respective methods on the tree ref
  instead.
