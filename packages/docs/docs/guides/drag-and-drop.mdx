...
In the following example, only items whose name starts with the letter __A__ are allowed to be dragged.
...
In the following example, only items whose name starts with the letter __A__ are considered viable drop targets.
    ...
    <UncontrolledTreeEnvironment
      ...
      canDropAt={(items, target) =>
        target.targetType === 'between-items'
          ? target.parentItem.startsWith('A')
          : target.targetItem.startsWith('A')
      }
    >
